...
- Image generation is now isolated from the main thread, speeding up the process significantly. Generation time varies mostly between 20ms and 2000ms, depending on user interaction and device performance. On the web, it runs inside separate web workers.
