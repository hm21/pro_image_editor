...
// Flutter imports:
import 'package:example/pages/video/widgets/video_send_bottom_bar.dart';
...
class CombineVideoEditorPages extends StatefulWidget {
  const CombineVideoEditorPages({
    super.key,
    required this.initialElement,
  });

  final ContentElement initialElement;

  @override
  State<CombineVideoEditorPages> createState() =>
      _CombineVideoEditorPagesState();
}

class _CombineVideoEditorPagesState extends State<CombineVideoEditorPages>
    with ExampleHelperState<CombineVideoEditorPages> {
  ...
  int _selectedPage = 0;
  ContentElement get _activeEditor => _editors[_selectedPage];
  final List<ContentElement> _editors = [];

  @override
  void initState() {
    _editors.add(widget.initialElement);
        ...
        body: Stack(
          ...
          children: [
            PageView(
              ...
              children: List.generate(_editors.length, (index) {
                var editor = _editors[index];
                return LayoutBuilder(
                  builder: (context, constraints) {
                    return ProImageEditor.memory(
                      editor.editorImage.byteArray!,
                      key: editor.imageEditorKey,
                      callbacks: ProImageEditorCallbacks(
                          ...
                          mainEditorCallbacks: MainEditorCallbacks(
                            ...
                            onScaleUpdate: (details) {
                              _whatsAppHelper.onScaleUpdate(
                                details,
                                editor.imageEditor!,
                              );
                            },
                            onScaleEnd: (details) => _whatsAppHelper.onScaleEnd(
                              details,
                              editor.imageEditor!,
                            ),
            ...
            if (_activeEditor.imageEditor != null &&
                (_activeEditor.imageEditor?.selectedLayerIndex ?? -1) < 0)
  ...
  List<Widget> _buildWhatsAppWidgets() {
    ...
    return [
      WhatsAppAppBar(
        configs: _activeEditor.imageEditor!.configs,
        onClose: _activeEditor.imageEditor!.closeEditor,
        onTapCropRotateEditor: _activeEditor.imageEditor!.openCropRotateEditor,
        onTapStickerEditor: () =>
            openWhatsAppStickerEditor(_activeEditor.imageEditor!),
        onTapPaintEditor: _activeEditor.imageEditor!.openPaintingEditor,
        onTapTextEditor: _activeEditor.imageEditor!.openTextEditor,
        onTapUndo: _activeEditor.imageEditor!.undoAction,
        canUndo: _activeEditor.imageEditor!.canUndo,
        openEditor: _activeEditor.imageEditor!.isSubEditorOpen,
      ),
      /*   if (_useMaterialDesign)
      ...
      VideoSendBottomBar(
        ...
        onRemoveItem: (index) {
          setState(() {
            ...
            if (_selectedPage >= _editors.length) {
              _selectedPage--;
            }
          });
        },
        ...
        onPickFile: (bytes, isVideo) {
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _pageController.jumpToPage(_editors.length - 1);
            setState(() {});
          });
        },
      ...
      WhatsappFilters(
        editor: _activeEditor.imageEditor!,
