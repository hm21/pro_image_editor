...
class FilterStateHistory {
  ...
  FilterStateHistory.fromMap(Map map)
      : filter = ColorFilterGenerator(
          ...
          filters: (map['filters'] as List<dynamic>?)?.map((e) => (e as List<dynamic>).map((e) => (e as num).toDouble()).toList()).toList() ?? [],
        ),
        opacity = num.tryParse(map['opacity']?.toString() ?? '1')?.toDouble() ?? 1;
